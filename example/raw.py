...
from tensorflow.keras.layers import Conv2D, ZeroPadding2D, Dropout, Flatten, Dense, Reshape, \
    LSTM
...
def gendata(fp, snr_choice):
    ...
    X_train_i = train_snr(snr_choice)
    Y_train_i = test_snr(snr_choice)
...
class CLDNN(Model):
    def __init__(self, in_shp, dropout, classes):
        ...
        self.f2 = Dense(len(classes), activation='softmax', kernel_initializer='he_normal', name="Dense1")
...
if __name__ == "__main__":
    ...
    model = CLDNN(in_shp=in_shp, dropout=dropout, classes=classes)
    model.compile(loss=tf.keras.losses.categorical_crossentropy,
                  optimizer=tf.keras.optimizers.Adam(lr=0.001),
                  metrics=['accuracy'])

    log_dir = prefix_dir + 'cldnn_' + '_' + str(epochs)
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)

    logs = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)
    ckpt_dir = prefix_dir + 'output/cldnn_%s/' % str_snr
    if not os.path.exists(ckpt_dir):
        os.makedirs(ckpt_dir)
    ckpt_file = ckpt_dir + 'cldnn_%s_%s_%s.ckpt' % (str_snr, str(dropout), str(epochs))
    ...
    plt.legend()
    logitPic = prefix_dir + "figures/cldnn/logit_%s_%s_%s.png" % (str(snr), str(dropout), str(epochs))
    ...
    confusionPic = prefix_dir + "figures/cldnn/confusion_%s_%s_%s.png" % (str(snr), str(dropout), str(epochs))
