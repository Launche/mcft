...
class CBLADNN(Model):
    def __init__(self, in_shp, dropout):
        ...
        # self.l1 = Bidirectional(LSTM(50, return_sequences=True))
        self.l1 = RnnLayer(128, dropout)
        self.a1 = RnnAttentionLayer(128, dropout)
...
if __name__ == "__main__":
    ...
    model = CBLADNN(in_shp=in_shp, dropout=dropout)
    model.compile(loss=tf.keras.losses.categorical_crossentropy,
                  optimizer=tf.keras.optimizers.Adam(lr=0.001),
                  metrics=['accuracy'])

    log_dir = prefix_dir + 'cbladnn_' + '_' + str(epochs)
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)

    logs = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)
    ckpt_dir = prefix_dir + 'output/cbladnn_%s/' % str_snr
    if not os.path.exists(ckpt_dir):
        os.makedirs(ckpt_dir)
    ckpt_file = ckpt_dir + 'cbladnn_%s_%s_%s.ckpt' % (str_snr, str(dropout), str(epochs))
    ...
    plt.legend()
    logitPic = prefix_dir + "figures/cbladnn/logit_%s_%s_%s.png" % (str(snr), str(dropout), str(epochs))
    ...
    confusionPic = prefix_dir + "figures/cbladnn/confusion_%s_%s_%s.png" % (str(snr), str(dropout), str(epochs))
